...
class Gadget(webapp.RequestHandler):
  ...
  def get(self):
    ...
    self.response.out.write(unicode(template.render(path, { 'docId' : docHash })))
...
class MainPage(webapp.RequestHandler):
  ...
  def get(self):
    ...
    if docHash is None or docHash == "":
      ...
      self.response.out.write(unicode(template.render(path, { 'uri' : uri })))
      ...
      path = os.path.join(os.path.dirname(__file__), template_file)
      self.response.out.write(unicode(template.render(path, { 'docId' : docHash , 'userName' : userName, 'newId' : newId, 'guidBase' : guidBase, 'nowMilliseconds': now, "sessionId" : sessionId, "isTest" : isTest, "isBlank" : isBlank, "isGadget" : isGadget, 'gadgetLibs' : gadgetLibs })))
...
class SaveDocument(webapp.RequestHandler):
  def get(self):
    ...
    if user:
      ...
      self.response.out.write(json.dumps({ 'error' : "no_login" }))
...
class FetchData(webapp.RequestHandler):
  def get(self):
    ...
    self.response.out.write(json.dumps(data))
    #else:
    #  self.response.out.write(json.dumps({error: "need login"}))
